...
class UsersViewTestCase(APITestCase):
    ...
    def test_register_without_username(self):
        response = self.client.post(reverse("api-login"), data={"password": "testuser2pass123",
                                                                "email": "test2@test.com"})
        self.assertEqual(response.status_code, 400)

    def test_register_without_password(self):
        response = self.client.post(reverse("api-login"), data={"username": "test_user2",
                                                                "email": "test2@test.com"})
        self.assertEqual(response.status_code, 400)
    ...
    def test_register_with_current_username(self):
        response = self.client.post(reverse("api-login"), data={"username": "test_user",
                                                                "password": "testuser2pass123",
                                                                "email": "test2@test.com"})
        self.assertEqual(response.status_code, 400)
