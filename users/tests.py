...
class ChangePasswordViewTestCase(APITestCase):
    ...
    def test_change_password_with_wrong_old_password(self):
        response = self.client.post(reverse("api-change-password"), data={"old_password": "wrongpassword",
                                                                          "new_password": "newpassword123"})
        self.assertEqual(response.status_code, 400)
        self.assertTrue("wrong" in response.content.decode('utf-8').lower())

    def test_change_password_with_short_password(self):
        response = self.client.post(reverse("api-change-password"), data={"old_password": "testuserpass123",
                                                                          "new_password": "1234"})
        self.assertEqual(response.status_code, 400)
